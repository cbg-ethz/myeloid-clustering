...
# ggtheme = theme_light(), # customize plot and risk table with a theme.
  ...
  # title="My title",
  size=0.7,
  # tables.theme = theme_survminer(font.main = 11, font.x = 11, font.y = 11),
  risk.table.y.text = FALSE # show bars instead of names in text annotations
  # in legend of risk table
); km_aml

km_aml$plot <- km_aml$plot +scale_y_continuous(
...
km_aml_riskscores <- ggsurvplot(
  ...
  # title="My title",
  size=0.7,
  # tables.theme = theme_cleantable(font.main = 11, font.x = 11, font.y = 11),
  risk.table.y.text = FALSE # show bars instead of names in text annotations
  # in legend of risk table
); km_aml_riskscores

km_aml_riskscores$plot <- km_aml_riskscores$plot +scale_y_continuous(
  breaks = c(0,0.2,0.4,0.6,0.8,1)
  # expand = c(0.01, 0)
)+scale_x_continuous(
  breaks = c(0,2,4,6,8,10)
  # expand = c(0.01, 0)
);km_aml_riskscores$plot
...
km_mds$plot <- km_mds$plot +scale_y_continuous(
  breaks = c(0,0.2,0.4,0.6,0.8,1)
  # expand = c(0.05, 0)
)+scale_x_continuous(
  breaks = c(0,2,4,6,8,10)
...
km_mds_riskscores$plot <- km_mds_riskscores$plot +scale_y_continuous(
  breaks = c(0,0.2,0.4,0.6,0.8,1)
  # expand = c(0.01, 0)
)+scale_x_continuous(
  breaks = c(0,2,4,6,8,10)
  # expand = c(0.01, 0)
);km_mds_riskscores$plot
...
km_aml_riskscores$plot <- km_aml_riskscores$plot + guides(colour = guide_legend(nrow = 2))

ggarrange(km_aml$plot, km_aml_riskscores$plot, km_aml$table, km_aml_riskscores$table, km_mds$plot, 
...
# pdf("~/Desktop/km_mds_aml_main_test.pdf", height = 10.7, width = 9,
#     family = "Arial", paper = "special", onefile = FALSE)
# # family = "Times New Roman", paper = "special", onefile = FALSE)
# op <- par(mar = c(5, 4, 0.05, 0.05) + 0.1)
# test <- arrange_ggsurvplots(list(km_aml, km_mds, km_aml_riskscores, km_mds_riskscores), print = TRUE, ncol = 2, nrow = 2, risk.table.height = 0.3)
# par(op)
# dev.off()
...
library("extrafont")
loadfonts()
pdf("~/Desktop/km_mds_aml_main.pdf", height = 9, width = 9,
    family = "Arial", paper = "special", onefile = FALSE)
# family = "Times New Roman", paper = "special", onefile = FALSE)
op <- par(mar = c(5, 4, 0.05, 0.05) + 0.1)
ggarrange(km_aml$plot, km_aml_riskscores$plot, km_aml$table+theme(plot.title = element_text(size=11)), km_aml_riskscores$table+theme(plot.title = element_text(size=11)), km_mds$plot,
          km_mds_riskscores$plot, km_mds$table+theme(plot.title = element_text(size=11)), km_mds_riskscores$table+theme(plot.title = element_text(size=11)), 
          nrow = 4, ncol = 2, heights = c(1,0.39,1,0.39), widths = c(1,1), labels = c("a","b","","","c","d","",""))#+theme(plot.margin = unit(c(0,5,0,0), 'lines'))
par(op)
dev.off()
...
library("extrafont")
loadfonts()
pdf("~/Desktop/km_mds_aml_main3.pdf", height = 7.7, width = 9,
    family = "Arial", paper = "special", onefile = FALSE)
# family = "Times New Roman", paper = "special", onefile = FALSE)
op <- par(mar = c(5, 4, 0.05, 0.05) + 0.1)
ggarrange(km_aml$plot, km_aml_riskscores$plot + guides(colour = guide_legend(nrow = 2)), km_mds$plot, 
          km_mds_riskscores$plot, nrow = 2, ncol = 2, heights = c(1,1), widths = c(1,1), labels = c("a","b","c","d"))
par(op)
dev.off()