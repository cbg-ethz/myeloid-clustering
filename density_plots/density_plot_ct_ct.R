...
# load the DAGs for each cancer type - learned via code above
maxfit <- readRDS("../results/cancer_type_networks.rds")
# maxfit <- readRDS("results/maxfit.rds")
...
for (k in 1:k_clust){
  # temp_mut_cov_data <- mut_cov_data[cancer_type==k-1,]
  allrelativeprobabs <- rep(0, dim(mut_cov_data)[1])
  allrelativeprobabs[cancer_type==k-1] <- 1
  ...
  scoresagainstclusters[,k] <- BiDAG::scoreagainstDAG(scorepar,maxfit[[k]]$DAG[-excluded,-excluded],mut_cov_data, bdecatCvec = apply(mut_cov_data, 2, function(x) length(unique(x))))
}
...
# for (k in 1:k_clust){
#   temp_mut_cov_data <- mut_cov_data[cancer_type==k-1,]
#   scorepar <- BiDAG::scoreparameters("bdecat", mut_cov_data, bdepar = bdepar)
#   scoresagainstclusters[,k] <- BiDAG::scoreagainstDAG(scorepar,maxfit[[k]]$DAG,mut_cov_data, bdecatCvec = apply(mut_cov_data, 2, function(x) length(unique(x))))
# }
...
# colourys2 <- alpha(c("#D7191C", "#FDAE61", "#ABDDA4", "#2B83BA"),0.3)
colourys2 <- c("#DD7788", "#117777", "#771122", "#DDDD77")

colourys3<-alpha(colourys2,0.6)
par(mar = c(0,0,0,0))
...
for(k in 1:k_clust){
  ...
  plot(z,display="filled.contour2",add=TRUE,cont=c(50),col=c("transparent",paste0(colourys2[k],"66")), alpha=0.3, drawpoints=FALSE,drawlabels=FALSE,lwd=1.5, bty="n")  
  
  points(textxs,textheights[k],col=colourys2[k],pch=19,cex=1.5)
  text(textxs,textheights[k],levelly2[k],pos=4)
...
pdf("~/Desktop/density_plot_ct_ct.pdf", width = 9, height = 5, onefile=FALSE)
transparent_plot
dev.off()
...
# Set parameters
excluded <- c(47:59) # mutations + cytogenetics only
# excluded <- c(48,55, 58:60)

# Load data
cancer_type <- read.table("../data/categorical-mutationCovariate-matrix.txt")[,58]
mut_cov_data <- read.table("../data/categorical-mutationCovariate-matrix.txt")[,-excluded]
string_edgepmat <- as.matrix(read.table("../data/string-edgepmat.txt"))[-excluded,-excluded]
blacklist_edgepmat <- as.matrix(read.table("../data/blacklist-edgepmat.txt"))[-excluded,-excluded]
...
# # # Perform BiDAG calculations
# # maxfit <- list()
# bdepar <- list(chi = 0.5, edgepf = 16)
# # for (i in 1:4){
# #   temp_mut_cov_data <- mut_cov_data[cancer_type==i-1,]
# #   temp_mut_cov_data[1,colSums(temp_mut_cov_data)==0] <- 1 # what is this line for!!!
# #   scorepar <- BiDAG::scoreparameters("bde", temp_mut_cov_data, edgepmat = string_edgepmat, bdepar = bdepar)
# #   maxfit[[i]] <- BiDAG::iterativeMCMC(scorepar, verbose=FALSE, blacklist=blacklist_edgepmat)
# # }
# # saveRDS(maxfit, "results/maxfit.rds")
# 
# # load the DAGs for each cancer type - learned via code above
# maxfit <- readRDS("results/maxfit.rds")
...
# Set colors and plotting parameters
colourys<-c("#202020","#771155","#114477","#ed2124","#771122","#DDDD77",
                     "#DDAA77","#117777","#CC99BB")
...
for(k in 1:k_clust){
  ...
  points(textxs,textheights[k],col=colourys3[k],pch=19,cex=1.5)
  text(textxs,textheights[k],levelly2[k],pos=4)
...
for(k in 1:4){
  ...
  points(reducey[selecteddots,1],reducey[selecteddots,2],col=colourys3[k],pch=19,cex=0.5)
...
pdf("~/Desktop/density_plot_ct_ct2.pdf", width = 9, height = 5, onefile=FALSE)
transparent_plot
dev.off()
